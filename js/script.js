(function ($) {
  ...
  function initTextAnimations() {
    ...
    // Check if device is mobile - disable animations on mobile
    var isMobile = window.matchMedia("(max-width: 768px)").matches;

    if (isMobile) {
      // On mobile, just show text immediately without animation
      $(
        ".text-anime-style-1, .text-anime-style-2, .text-anime-style-3, .text-anime-style-4, .text-anime-style-5, .text-anime-style-6, .text-anime-style-7"
      ).css({
        opacity: "1",
        visibility: "visible",
      });
